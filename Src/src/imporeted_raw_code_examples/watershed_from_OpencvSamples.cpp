...
{
    if( x < 0 || x >= img__.cols || y < 0 || y >= img__.rows )
        return;
    ...
    else if( event == EVENT_MOUSEMOVE && (flags & EVENT_FLAG_LBUTTON) )
    {
        ...
        line( img__, prevPt, pt, Scalar::all(255), 5, 8, 0 );
        prevPt = pt;
        imshow("image", img__);
    }
}

int mainWSH( Mat img0)  /// int argc, char** argv )
{
...
/*
    ...
    img0.copyTo(img__);
    cvtColor(img__, markerMask  , COLOR_BGR2GRAY);
